...
try:
    data = pd.read_csv("TrainitemByWeek\{}ByWeek.csv".format(10117), parse_dates=[0], index_col=0, date_parser=parser)
    
    data["OrderQty"] = [float(x) for x in data["OrderQty"]]
    #alpha = 1.3
    #window = 15
    print('processing item {0}'.format(10117))
    item_output = []
    ...
    with open("output\\MA_%.0f.csv" %10117, 'a', newline='') as f:
        spamwriter = csv.writer(f)
